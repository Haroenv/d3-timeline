  ...
  d3.timeline = function() {
    ...
    function timeline (gParent) {
      ...
      g.each(function(d, i) {
        ...
        d.forEach( function(datum, index){
          var data = datum.times,
              hasLabel = (typeof(datum.label) != 'undefined');

          // issue warning about using id per data set. Ids should be individual to data elements
          if (typeof(datum.id) != 'undefined' && window.console) {
            console.warn('d3Timeline Warning: Ids per dataset is deprecated in favour of a class key. Ids are now per data element.');
          }
      ...
      var belowLastItem = (margin.top + (itemHeight + itemMargin) * maxStack),
          aboveFirstItem = margin.top,
          timeAxisYPosition = showAxisTop ? aboveFirstItem : belowLastItem;
